...
export class AutonomousSessionManager {
  ...
  async getUserSessions(userId: string): Promise<AutonomousSession[]> {
    return Array.from(this.sessions.values())
      .filter((session: unknown) => session.userId === userId)
      .sort((a, b) => b.lastActivity.getTime() - a.lastActivity.getTime());
  }

  async getActiveSessions(): Promise<AutonomousSession[]> {
    return Array.from(this.sessions.values()).filter((session: unknown) => session.status === "active");
  }
  ...
  async getSessionMetrics(sessionId: string): Promise<SessionMetrics | null> {
    ...
    const responseTimes = messages.filter((msg: unknown) => msg.role === "assistant").map(() => 200 + Math.random() * 300); // Stub response times
  ...
  getActiveSessionCount(): number {
    return Array.from(this.sessions.values()).filter((session: unknown) => session.status === "active").length;
  }
}
