...
export class ResolutionDetector {
  ...
  private detectExplicitSignals(messages: ConversationContext["messages"]): ResolutionSignal[] {
    ...
    messages.forEach((message: unknown) => {
      if (message.role === "user") {
        ...
        const foundPhrases = explicitPhrases.filter((phrase: unknown) => content.includes(phrase));
  ...
  private detectImplicitSignals(messages: ConversationContext["messages"]): ResolutionSignal[] {
    ...
    messages.forEach((message: unknown) => {
      if (message.role === "user") {
        const content = message.content.toLowerCase();
        const foundIndicators = positiveIndicators.filter((indicator: unknown) => content.includes(indicator));
  ...
  private detectBehavioralSignals(context: ConversationContext): ResolutionSignal[] {
    ...
    if (recentMessages.length >= 3) {
      const userMessages = recentMessages.filter((m: unknown) => m.role === "user");
  ...
  private calculateOverallConfidence(signals: ResolutionSignal[]): number {
    ...
    const explicitSignals = signals.filter((s: unknown) => s.type === "explicit");
    const implicitSignals = signals.filter((s: unknown) => s.type === "implicit");
    const behavioralSignals = signals.filter((s: unknown) => s.type === "behavioral");
