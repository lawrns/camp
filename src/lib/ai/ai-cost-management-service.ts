...
export class AICostManagementService {
  ...
  async generateBillingPeriodSummary(
    ...
    if (aiUsage) {
      aiUsage.forEach((log: unknown) => {
        ...
        modelBreakdown[model].tokens += (log.input_tokens || 0) + (log.output_tokens || 0);
        modelBreakdown[model].cost += log.processing_cost || 0;
      });
    }
  ...
  async generateOptimizationRecommendations(organizationId: string, daysBack: number): Promise<any[]> {
    ...
    aiUsage.forEach((log: unknown) => {
      ...
      modelStats[model].cost += log.processing_cost || 0;
      modelStats[model].tokens += (log.input_tokens || 0) + (log.output_tokens || 0);
    });
