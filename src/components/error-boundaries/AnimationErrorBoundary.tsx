...
function AnimationErrorFallback({ error, resetErrorBoundary, componentName }: AnimationErrorFallbackProps) {
  ...
  const isDynamicImportError =
    (error instanceof Error ? error.message : String(error)).includes("dynamically imported module") || (error instanceof Error ? error.message : String(error)).includes("Cannot resolve module");
        ...
        <div className="min-w-0 flex-1">
          ...
          {process.env.NODE_ENV === "development" && (
            <details className="mt-2">
              ...
              <pre className="mt-1 whitespace-pre-wrap text-tiny text-red-600">{(error instanceof Error ? error.message : String(error))}</pre>
            </details>
          )}
          <div className="mt-3">
            <button
