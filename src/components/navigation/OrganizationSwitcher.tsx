...
export function OrganizationSwitcher({ currentOrganization, className }: OrganizationSwitcherProps) {
  ...
  const loadOrganizations = async () => {
    try {
      ...
      if (error) {
        throw new Error(`Failed to load organizations: ${(error instanceof Error ? error.message : String(error))}`);
      }
  ...
  const switchOrganization = async (organizationId: string) => {
    try {
      ...
      toast.error(error instanceof Error ? (error instanceof Error ? error.message : String(error)) : "Failed to switch organization");
    } finally {
      setSwitching(false);
    }
  };
  ...
  const createOrganization = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? (error instanceof Error ? error.message : String(error)) : "Failed to create organization");
    } finally {
      setCreating(false);
    }
  };
