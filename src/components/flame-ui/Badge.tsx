...
const badgeVariants = cva(
  ...
  [
    ...
    "text-[var(--ds-font-size-xs)] font-[var(--ds-font-weight-semibold)]",
    "transition-all duration-[var(--ds-duration-200)] ease-[var(--ds-ease-out)]",
  ...
  ],
  {
    variants: {
      ...
      size: {
        sm: "px-[var(--ds-spacing-2)] py-[0.125rem] text-[0.625rem]",
        default: "px-[var(--ds-spacing-3)] py-[var(--ds-spacing-1)] text-[var(--ds-font-size-xs)]",
        lg: "px-[var(--ds-spacing-4)] py-[var(--ds-spacing-2)] text-[var(--ds-font-size-sm)]",
      },
      interactive: {
  ...
  ({ className, variant, size, interactive, icon, onRemove, children, ...props }, ref) => {
    ...
    return (
      <div
        ...
        {...props}
      >
        {icon && <span className="mr-[var(--ds-spacing-1)] flex items-center flex-shrink-0">{icon}</span>}
        <span className="whitespace-nowrap truncate">{children}</span>
        {onRemove && (
          <button
            type="button"
            className={cn(
              "ml-[var(--ds-spacing-2)] -mr-[var(--ds-spacing-1)]",
              "flex items-center justify-center",
              "w-[var(--ds-spacing-4)] h-[var(--ds-spacing-4)]",
              "ds-rounded-ds-full",
              "hover:bg-black/10 dark:hover:bg-white/10",
              "focus:outline-none focus:ring-1 focus:ring-[var(--ds-color-focus)]",
              "transition-colors duration-[var(--ds-duration-150)]"
            )}
            onClick={(e) => {
              e.stopPropagation();
              onRemove();
...
>(({ className, gap = "default", ...props }, ref) => {
  const gapClasses = {
    sm: "gap-[var(--ds-spacing-1)]",
    default: "gap-[var(--ds-spacing-2)]",
    lg: "gap-[var(--ds-spacing-3)]",
  };
