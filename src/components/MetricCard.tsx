      ...
      <Card className="relative overflow-hidden">
        ...
        <CardContent>
          ...
          {trend && !loading && (
            ...
            >
              <OptimizedMotion.div
                animate={trend.isUp ? { y: [-2, 0, -2] } : { y: [2, 0, 2] }}
                transition={{ duration: 2, repeat: Infinity }}
                className={cn(
                  "text-typography-sm flex items-center gap-1",
                  trend.isGood ? "text-semantic-success-dark" : "text-red-600"
                )}
              >
                {trend.isUp ? <TrendUp className="h-4 w-4" /> : <TrendDown className="h-4 w-4" />}
