...
export class WidgetErrorBoundary extends Component<Props, State> {
  ...
  override componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error('[WidgetErrorBoundary] Widget error caught:', error);
    console.error('[WidgetErrorBoundary] Error stack:', error.stack);
  ...
  private isChunkLoadError(error: Error): boolean {
    return (
      ...
      (error instanceof Error ? error.message : String(error)).includes("Loading CSS chunk") ||
      (error instanceof Error ? error.message : String(error)).includes("Failed to import") ||
      (error instanceof Error ? error.message : String(error)).includes("dynamically imported module")
    );
  }
              ...
              {process.env.NODE_ENV === "development" && this.state.error && (
                <details className="mt-3">
                  ...
                  <div className="bg-background mt-2 rounded p-spacing-sm font-mono text-tiny">
                    <div>
                      <strong>Error:</strong> {this.state.error instanceof Error ? this.state.error.message : String(this.state.error)}
                    </div>
                    <div>
