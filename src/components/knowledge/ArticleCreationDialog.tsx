...
import { Check, FileText, Loader2, Sparkles, Tag, X } from "lucide-react";
...
export function ArticleCreationDialog({
  ...
  const generateDraft = async () => {
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? (error instanceof Error ? error.message : String(error)) : "Failed to generate draft",
        variant: "destructive",
      });
    } finally {
      setGenerating(false);
  ...
  const saveDraft = async () => {
    try {
      ...
      toast({
        title: "Save Failed",
        description: error instanceof Error ? (error instanceof Error ? error.message : String(error)) : "Failed to save article",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
