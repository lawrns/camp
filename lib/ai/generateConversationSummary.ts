...
const constructAnthropicMessagesForConversationSummary = (
  ...
  const allEmailsContent = emails
    .map(
      (email) =>
        `From: ${email.role && HELPER_TO_AI_ROLES_MAPPING[email.role] ? HELPER_TO_AI_ROLES_MAPPING[email.role] : "Unknown"}\nContent: ${cleanUpTextForAI(email.cleanedUpText ?? "")}`
    )
    .join("\n\n");
...
export const generateConversationSummary = async (conversationId: string, { force }: { force?: boolean } = {}) => {
  ...
  const emails = await db.query.conversationMessages.findMany({
    where: and(
      ...
      inArray(conversationMessages.role, ["user", "assistant", "system"]), // Use valid role values from schema
