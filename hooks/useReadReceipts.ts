...
interface RealtimeChannel {
  on: (event: string, options: { event: string }, callback: (payload: { payload: ReadReceiptPayload }) => void) => void;
  subscribe: () => void;
  unsubscribe: () => void;
}
...
export function useReadReceipts({
  ...
  const fetchReadReceipts = useCallback(async () => {
    ...
    try {
      ...
      receipts?.forEach((receipt: DatabaseReadReceipt) => {
  ...
  useEffect(() => {
    ...
    channelRef.current.on("broadcast", { event: "read_receipt" }, (payload: { payload: ReadReceiptPayload }) => {
