...
export async function POST(request: NextRequest) {
  try {
    ...
    const sentiment = sentimentAnalysis.sentiment.compound > 0 ? 'positive' : sentimentAnalysis.sentiment.compound < 0 ? 'negative' : 'neutral';
...
async function generateSuggestions({
  ...
  try {
    ...
    try {
      ...
      const aiSuggestions = JSON.parse(responseContent);
