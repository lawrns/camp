...
const UnifiedInboxDashboard: React.FC<UnifiedInboxDashboardProps> = ({ className = "" }) => {
  ...
  // TEMPORARY: For testing, use fallback organization if user auth is not working
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      let query = supabaseClient
        ...
        .eq("organization_id", effectiveUser.organizationId!)
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const messageData = {
        ...
        sender_type: "agent",
        sender_name: effectiveUser?.email || "Agent",
  ...
  useEffect(() => {
    if (!effectiveUser?.organizationId) return;
    ...
    const channelName = selectedConversation
      ? UNIFIED_CHANNELS.conversation(effectiveUser.organizationId, selectedConversation.id.toString())
      : UNIFIED_CHANNELS.organization(effectiveUser.organizationId);
    ...
    const messagesChannel = supabaseClient
      ...
      .on(
        "postgres_changes",
        {
          ...
          schema: "public",
          table: "messages",
          filter: `organization_id=eq.${effectiveUser.organizationId}`,
        },
        (payload: any) => {
    ...
    const conversationsChannel = supabaseClient
      .channel(UNIFIED_CHANNELS.organization(effectiveUser.organizationId))
      .on(
        "postgres_changes",
        {
          event: "INSERT",
      ...
      // Also listen for broadcast events from widget
      .on("broadcast", { event: UNIFIED_EVENTS.CONVERSATION_CREATED }, () => {
        // Refresh conversations when new ones are created via widget
        loadConversations();
      })
      .on("broadcast", { event: UNIFIED_EVENTS.CONVERSATION_UPDATED }, () => {
  ...
  }, [effectiveUser?.organizationId, selectedConversation?.id, loadConversations]);
