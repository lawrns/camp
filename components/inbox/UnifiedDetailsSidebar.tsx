...
import { Activity, ArrowSquareOut, Calendar, Clock, Copy, PencilSimple as Edit2, Envelope as Mail, MapPin, MessageCircle as MessageSquare, Phone, User, X,  } from "lucide-react";
...
export function DetailsSidebar({ isOpen, onClose, conversationId, conversation, className }: DetailsSidebarProps) {
  ...
  const customerData = {
    ...
    avatar: getAvatarUrl(conversation?.customerEmail) || "",
    location: conversation?.customer_data?.location || "",
    lifetime_value: conversation?.orders ? calculateLTV(conversation.orders) : "$0",
    total_conversations: conversation?.total_count || 0,
    satisfaction_score: conversation?.satisfaction_score || "N/A",
  ...
  return (
    <div className="space-y-6 spacing-3">
      ...
      <OptimizedMotion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center">
        <Avatar src={customer.avatar} name={customer.name} className="mx-auto mb-3 h-20 w-20" />
...
// Export as UnifiedDetailsSidebar for the import
