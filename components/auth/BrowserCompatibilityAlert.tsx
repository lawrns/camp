...
export function BrowserCompatibilityAlert({
  ...
  const getSeverityColor = (severity: "info" | "warning" | "error") => {
    switch (severity) {
      case "error":
        return "border-[var(--fl-color-danger-muted)] bg-[var(--fl-color-danger-subtle)] text-red-800";
      case "warning":
        return "border-orange-200 bg-orange-50 text-orange-800";
      case "info":
      default:
        return "border-[var(--fl-color-info-muted)] bg-[var(--fl-color-info-subtle)] text-blue-800";
    }
  };
  ...
  const getSeverityIcon = (severity: "info" | "warning" | "error") => {
    switch (severity) {
      case "error":
        return <Icon icon={AlertTriangle} size={20} className="text-red-600" />;
      case "warning":
        return <Icon icon={AlertTriangle} size={20} className="text-orange-600" />;
      case "info":
      default:
        return <Icon icon={Info} size={20} className="text-blue-600" />;
    }
  };
                  ...
                  {report.storageEnvironment ? (
                    <div className="space-y-1">
                      <div className="flex items-center gap-ds-2 text-sm">
                        <div
                          className={`h-2 w-2 rounded-ds-full ${
                            report.storageEnvironment.indexedDBWorking ? "bg-semantic-success" : "bg-brand-mahogany-500"
                          }`}
                        />
                        <span>
                      ...
                      <div className="flex items-center gap-ds-2 text-sm">
                        <div
                          className={`h-2 w-2 rounded-ds-full ${
                            report.storageEnvironment.localStorageWorking
                              ? "bg-semantic-success"
                      ...
                      <div className="flex items-center gap-ds-2 text-sm">
                        <div
                          className={`h-2 w-2 rounded-ds-full ${
                            report.storageEnvironment.sessionStorageWorking
                              ? "bg-semantic-success"
                              : "bg-brand-mahogany-500"
                          }`}
                        />
                      ...
                      {report.detectedExtensions
                        ...
                        .map((ext, index) => (
                          <div key={index} className="flex items-start gap-ds-2 text-sm">
                            <div
                              className={`mt-1.5 h-2 w-2 rounded-ds-full ${
                                ext.impact === "high"
                                  ? "bg-brand-mahogany-500"
                                  : ext.impact === "medium"
                                    ? "bg-semantic-warning"
