...
const skeletonVariants = cva(
  // Base styles
  ["rounded-[var(--fl-rounded-ds-md)]", "bg-[var(--fl-color-background-muted)]", "relative overflow-hidden"],
  {
    variants: {
      variant: {
        default: "",
...
const SkeletonCard = React.forwardRef<HTMLDivElement, SkeletonCardProps>(
  ({ className, showAvatar = true, showActions = false, showImage = false, lines = 3, variant, animation }, ref) => {
    return (
      ...
      >
        {showImage && (
          <Skeleton variant={variant} animation={animation} className="h-48 w-full rounded-[var(--fl-rounded-ds-md)]" />
        )}
...
export { Skeleton, SkeletonAvatar, SkeletonButton, SkeletonCard, SkeletonTable, SkeletonText, skeletonVariants };
