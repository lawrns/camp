...
jest.mock('../../components/InboxDashboard/sub-components/EmojiPicker', () => ({
  default: () => <div data-testid="emoji-picker" />
}));

jest.mock('../../components/InboxDashboard/sub-components/TemplatePanel', () => ({
  default: () => <div data-testid="template-panel" />
}));

jest.mock('../../components/InboxDashboard/sub-components/AISuggestionsPanel', () => ({
  default: () => <div data-testid="ai-suggestions-panel" />
}));

jest.mock('../../components/InboxDashboard/sub-components/AttachmentPreview', () => ({
...
describe('Composer Component', () => {
  ...
  describe('Message Input', () => {
    it('updates message when typing', async () => {
      ...
      // Check that setNewMessage was called multiple times (once per character)
      expect(setNewMessage).toHaveBeenCalledTimes(11); // "Hello world" has 11 characters
      // The last call should be with the complete string
      expect(setNewMessage).toHaveBeenNthCalledWith(11, 'Hello world');
    });
  ...
  describe('Internal Notes', () => {
    it('submits note when in note mode', async () => {
      const user = userEvent.setup();
      const { addNote } = require('@/lib/data/note');
  ...
  describe('AI Features', () => {
    it('toggles AI suggestions panel', async () => {
      ...
      const generateAISuggestions = jest.fn();
      render(<Composer {...defaultProps} setShowAISuggestions={setShowAISuggestions} generateAISuggestions={generateAISuggestions} />);
  ...
  describe('Error Handling', () => {
    it('handles note submission errors gracefully', async () => {
      const user = userEvent.setup();
      const { addNote } = require('@/lib/data/note');
      addNote.mockRejectedValueOnce(new Error('Note submission failed'));
