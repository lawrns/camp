...
// import { checkRepositoryIssuesEnabled, listRepositories } from "@/lib/github/client"; // Module not found

// Simple fallbacks for GitHub functions
const listRepositories = (installationId: string) => {
  return Promise.resolve([]); // Fallback: no repositories
};

const checkRepositoryIssuesEnabled = (params: { installationId: string; owner: string; repo: string }) => {
  return Promise.resolve(true); // Fallback: assume issues are enabled
};
...
export const githubRouter = {
  ...
  disconnect: mailboxProcedure.mutation(async ({ ctx }) => {
    await disconnectGitHub(ctx.mailbox.id.toString());
  }),
  updateRepo: mailboxProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await updateGitHubRepo(ctx.mailbox.id.toString(), input.repoOwner, input.repoName);
    }),
};
